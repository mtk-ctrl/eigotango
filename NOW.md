...
- **画面構成（IA）**: 共通ボトムナビ（🏠ホーム `/home` ／📈きろく `/progress` ／⚙️せってい `/settings`）。1画面1役割。
  - `/home` は役割別の着地点（生徒＝今日の学習ヒーロー＋復習状況／親＝子どもの状況＋自分の学習）。旧 `/parent` ダッシュボードは `/home` に統合（リダイレクト）。
  - 設定・1日の問題数・プラン・通知は `/settings` に集約。学習中（`/study`）はナビ非表示の集中モード。
