...
| 日付 | 内容 |
|---|---|
| 2026-07-02 | モチベーション強化: 学習完了画面に『🚀もう◯語おぼえる！』ボタン（/studyは毎回次の未学習バッチを出すので設定数を超えて何周でも継続可能。復習完了には『新しい単語もやる』）。結果カードに連続正解コンボ（🔥nれんぞく）＋褒め言葉ランダム化。生徒ホームの新しい単語カードに『今日はもう◯語おぼえた』表示、統計タイルを『✅今日おぼえた』（超過分も積み上がる）に |
| 2026-07-02 | 実使用フィードバック対応: 「今日」タブに『✅今日おぼえたn語』の積み上げ表示を追加（従来は残り新規語のみで頑張った分が見えなかった。タブは✓済+残り表示）。親ホームの単語/復習リストに『自分の』『◯◯さんの』と名前を明示し、子どもの単語リスト・復習リストも親ホームから閲覧可能に（getDailyWordsにtodayDone追加） |
