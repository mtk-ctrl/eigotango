...
// Edge Function(batch-notify) が LINE 未設定でデプロイされない問題を回避するため Next 側で実装。
...
export async function POST(req: Request) {
  ...
  const studentMap = new Map<string, ProfileRow & { name: string }>()
  ...
  if (managedKids.length > 0) {
    ...
    for (const kid of managedKids) {
      ...
      targets.push({ email: parent.email, name: kid.name })
    }
  }

  const appUrl = process.env.NEXT_PUBLIC_APP_URL ?? 'https://eigotango.mtk551141.workers.dev'
  let sent = 0
